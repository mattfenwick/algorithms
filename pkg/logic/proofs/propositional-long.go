...
var propositionalLongProofSections = []*ProofsSection{
	NewProofsSection("theorems",
		...
		NewRootProof("~ ( P -> Q ) <-> ( P ^ ~ Q )",
			...
			IBiconditional(Not(Implication(P, Q)), And(P, Not(Q))),
		),
		...
		NewRootProof("( P v Q ) <-> ( ~ P -> Q )",
			...
			IBiconditional(Or(P, Q), Implication(Not(P), Q)),
		),
		NewRootProof("~ ( P v Q ) <-> ( ~ P ^ ~ Q )",
			...
			IBiconditional(Not(Or(P, Q)), And(Not(P), Not(Q))),
		),
		NewRootProof("~ ( P ^ Q ) <-> ( ~ P v ~ Q )",
	...
	NewProofsSection("arrows",
		...
		NewRootProof("( ( P ^ Q ) -> R ) <-> ( P -> ( Q -> R ) )",
			...
			IBiconditional(Implication(And(P, Q), R), Implication(P, Implication(Q, R))),
		),
		...
		NewRootProof("( ( P v Q ) -> R ) <-> ( ( P -> R ) ^ ( Q -> R ) )",
			NewProofImplication(Implication(Or(P, Q), R),
				...
				IAnd(Implication(P, R), Implication(Q, R)),
			),
			NewProofImplication(And(Implication(P, R), Implication(Q, R)),
				NewProofImplication(Or(P, Q),
					&Reiterate{Term: And(Implication(P, R), Implication(Q, R))},
					EAnd(Implication(P, R), Implication(Q, R), true),
					EAnd(Implication(P, R), Implication(Q, R), false),
					EOr(P, Q, R),
				),
			),
		...
		NewRootProof("( ( P -> R ) v ( Q -> R ) ) <-> ( ( P ^ Q ) -> R )",
			NewProofImplication(Or(Implication(P, R), Implication(Q, R)),
				...
				NewProofImplication(Implication(Q, R),
					NewProofImplication(And(P, Q),
						&Reiterate{Term: Implication(Q, R)},
						EAnd(P, Q, false),
						EImply(Q, R),
					),
				),
	...
	NewProofsSection("associativity",
		NewRootProof("( ( P ^ Q ) ^ R ) <-> ( P ^ ( Q ^ R ) )",
			...
			IBiconditional(And(And(P, Q), R), And(P, And(Q, R))),
		),
		NewRootProof("( ( P v Q ) v R ) <-> ( P v ( Q v R ) )",
			...
			IBiconditional(Or(Or(P, Q), R), Or(P, Or(Q, R))),
		),
		NewRootProof("( ( P <-> Q ) <-> R ) <-> ( P <-> ( Q <-> R ) )",
			...
			IBiconditional(Biconditional(Biconditional(P, Q), R), Biconditional(P, Biconditional(Q, R))),
		),
	),
